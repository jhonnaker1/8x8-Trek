...
const Panel panels40[PANEL_COUNT] = {
...
static void draw_panel40(const Panel *p, const char *title) {
    ...
    if (title)
        scr_puts((unsigned char)(p->x + 2), p->y, title, TITLE_COL);
}
...
void draw_console40(unsigned char page, const char *const *titles) {
    ...
    for (i = 0; i < PANEL_COUNT; i++) {
        if (panel40_page[i] != page) continue;
        draw_panel40(&panels40[i], titles ? titles[i] : (const char *)0);
    }
