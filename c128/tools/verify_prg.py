...
def check_briefing():
    ...
    # AND THE 40-COLUMN ONE, off the same rule. It is generated by
    # tools/reflow_briefing.py, and a generated file that nobody checks is a
    # generated file that drifts the first time its generator changes.
    f40 = C128 / "src" / "briefing40.txt"
    if f40.exists():
        bad40 = []
        for n, page in enumerate(f40.read_text().split("\f"), 1):
            ls = page.strip("\n").split("\n")
            w = max(len(l) for l in ls)
            if len(ls) > BRIEF_ROWS or w > 38:
                bad40.append((n, len(ls), w))
        if bad40:
            die("40-column briefing pages that will not fit:\n" + "\n".join(
                "         page %d: %d lines (max %d), %d columns (max 38)"
                % (n, h, BRIEF_ROWS, w) for n, h, w in bad40))
        print("verify: %d 40-column briefing pages, all inside 38x%d -- ok"
              % (len(f40.read_text().split("\f")), BRIEF_ROWS))
