 ...
 *
 * ADAPTED FROM c128/src/storage.c, which is where all the hard-won parts of
 * this came from -- the command-channel rule, the write-then-status order,
 * the scratch-before-write. Every KERNAL call it uses exists on the X16, which
 * is why this port was chosen next.
 *
 * WHAT THE C128 NEEDED AND THIS DOES NOT: SETBNK. The C128 KERNAL asks which
 * RAM bank the filename and data live in and defaults to bank 15, where this
 * port's buffers are BASIC ROM rather than RAM -- skip it there and a read
 * appears to work while filling the buffer with fragments of BASIC. The X16
 * has no such question for KERNAL file I/O, so set_banks() is gone rather
 * than carried across as a no-op.
 *
 * DEVICE 8 IS HOSTFS UNDER THE EMULATOR. x16emu intercepts the IEEE API and
 * serves device 8 from a host directory unless -sdcard is given, so the same
 * code path works against real files during development and against an SD
 * card image later. Nothing here knows the difference.
 */
