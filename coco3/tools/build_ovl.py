...
# The stack lives just under the I/O page, growing down into the gap above the
# overlay window. 1K is what cmoc's own crt assumes.
STACK_TOP    = 0xFE00
STACK_BYTES  = 1024
...
def place_stack(spath):
    ...
    1. MASK INTERRUPTS FIRST. The CoCo's 60Hz IRQ vectors through Disk BASIC's
       handler, and that handler resets S to BASIC's own stack -- which on this
       machine sits around $3400, inside this port's code. Traced on hardware:
       20ms after the program started, PC was at $88EA in ROM and S had gone
       from $FE00 to $34F4, and from then on every push wrote over the port.
       Doing this inside vdc_init() was far too late; the first tick beat it.

    2. THEN THE STACK. cmoc's CoCo runtime places it from Disk BASIC's memory
       pointers, and this port takes the machine away from BASIC.

    3. THEN ALL-RAM MODE. The CoCo 3 boots with ROM over $8000-$FEFF and this
       image reaches $B82F resident with its window above that, so most of the
       program and all of the window are underneath ROM until $FFDF is
       written. It must be a CPU write: these are write-only address latches
       and a debugger poke does nothing, which made a correct diagnosis look
       wrong for two runs.

    S is set AGAIN after INILIB, because INILIB positions a stack of its own
    from the same BASIC pointers."""
    s = open(spath).read()
    if "LBSR\tINILIB" not in s:
        sys.exit("build_ovl: no INILIB call in %s -- cannot take the machine" % spath)
    # 4. SAFE INTERRUPT VECTORS. ORCC masks IRQ and FIRQ; IT CANNOT MASK NMI,
    #    and the disk controller raises one on every completed operation. The
...
def blocks(path):
    """The (addr, bytes) blocks of a DECB binary, and its exec address."""
    d = open(path, "rb").read()
    i, out, exec_ad = 0, [], 0
    while i + 5 <= len(d):
...
def write_decb(path, blks, exec_ad):
    """A DECB binary: a 5-byte header per block, then a $FF trailer carrying
    the exec address."""
    with open(path, "wb") as f:
        for ad, data in blks:
            f.write(struct.pack(">BHH", 0x00, len(data), ad))
            f.write(data)
        f.write(struct.pack(">BHH", 0xFF, 0, exec_ad))


def write_map(imgdir, win, names, idx_label=(), slots=1, ceiling=None):
    ...
    with open(os.path.join(imgdir, "ovlmap.h"), "w") as f:
        ...
        # NOT to $FF00. The stack lives just under the I/O page and grows
        # down; an ovl_load told it had that much room would read an image
        # straight through it.
        # AND NOT ALWAYS TO THE STACK EITHER. OVL_SIZE is the bound handed to
        # plat_read_all, so it is the only thing standing between an oversized
        # image and whatever sits above the window. The card port has nothing
        # up there; the card-less one keeps its 2K message log at $F200, and a
        # bound of "everything up to the stack" would have read an overlay
        # straight through it the first time one grew past the gap. A port
        # with a tenant of its own passes --ceiling.
        f.write("#define OVL_SIZE    %d   /* to $%04X */\n"
                % (ceiling - win, ceiling))
...
def main():
    ...
    # The top of what an overlay image may occupy. Defaults to the stack
    # reserve, which is right for a port with nothing else up there; a port
    # that keeps a tenant above the window (coco3gime's message log at $F200)
    # passes its base, so OVL_SIZE cannot bound a read straight through it.
    ...
    ceiling = int(a.ceiling, 16) if a.ceiling else STACK_TOP - STACK_BYTES
    ...
    write_map(a.imgdir, win, names, idx_label, slots, ceiling)
    ...
    if names:
        ...
        if placed != win:
            # THE MESSAGE USED TO SAY "is below the top of bss" WHATEVER HAD
            # HAPPENED, including when the requested window was comfortably
            # ABOVE it and simply got packed down -- which reads as an error
            # about the wrong thing. The window is always pinned to the first
            # page above bss; say which way it moved.
            print("  window       $%04X %s the first page above bss ($%04X); "
                  "using $%04X" % (win, "is below" if win < placed else
                                   "leaves a gap over", top, placed))
            win = placed
            write_map(a.imgdir, win, names, idx_label, slots, ceiling)
    ...
    # THE RESIDENT BINARY IS WHAT THE MACHINE LOADS, so write it without the
    # window blocks. Left in, all eleven images would load one over another at
    # the same address and the last one would win -- which is the bug this
    # separation exists to prevent, not a tidy-up.
    write_decb(a.out, resident, exec_ad)

    # THE BOOTSTRAP ASSERTION. vdc_init() switches the machine out of ROM/RAM
    # mode; until it runs, the CoCo 3 has ROM over $8000-$FEFF. So every byte
    # of the path that REACHES it -- program_start, INILIB, main()'s prologue
    # and vdc_init itself -- must live below $8000, or main() calls into Color
    # BASIC ROM and the 6809 never comes back. It did: _vdc_init landed at
    # $A2D8 and the program was last seen at $A03F inside the ROM. Link order
    # is what places it, so the order is load-bearing and asserted here rather
    # than left to a comment.
    for name in ("program_start", "_main", "_vdc_init"):
        for line in open(a.out + ".map"):
            ...
            if m:
                ad = int(m.group(1), 16)
                if ad >= 0x8000:
                    sys.exit("build_ovl: %s is at $%04X, inside the ROM the "
                             "machine boots with. It must be below $8000 or it "
                             "cannot be reached before vdc_init() switches to "
                             "all-RAM mode. Check the link order in RES_SRC."
                             % (name, ad))
                break
    ...
    print("  stack        $%04X down, %d bytes reserved to $%04X"
          % (STACK_TOP, STACK_BYTES, limit))
    print("  free below $%04X: %d" % (limit, limit - top))
    if top > limit:
        sys.exit("build_ovl: the image overruns the stack reserve at $%04X by %d"
                 % (limit, top - limit))
