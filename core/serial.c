...
uint16_t trek_state_save(uint8_t *buf, uint16_t max) {
    ...
    put8(ship.orbiting); put16(ship.rescues); put16(ship.stars_gone); put16(ship.wear_last); put8(ship.lost_how);
...
uint8_t trek_state_load(const uint8_t *buf, uint16_t len) {
    ...
    ship.orbiting = get8(); ship.rescues = get16(); ship.stars_gone = get16(); ship.wear_last = get16(); ship.lost_how = get8();
