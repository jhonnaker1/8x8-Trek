...
def write_blob(by_id, path):
    """Write the pool file from an id -> text map.

    THE FORMAT IS DOCUMENTED AT THE CALL SITE IN main(): a 16-bit count, then
    that many 16-bit offsets, then the text with each string NUL terminated,
    then 64 bytes of padding so a full-width read of the last string cannot run
    off the end. """
    off, offsets, text = 0, [], bytearray()
    for i in sorted(by_id):
        offsets.append(off)
        b = by_id[i].encode("ascii", "replace") + b"\0"
        text += b
        off += len(b)
    blob = bytearray()
    blob += len(by_id).to_bytes(2, "little")
    for o in offsets:
...
def override_blob(over_path, out_path):
    """Re-emit the pool with a few ids carrying different text.

    WHY THIS EXISTS. Two of the 326 strings name the machine -- the version bar
    and the last line of the credit panel -- and a second port has to say
    something else. Forking strings.txt to change two lines would mean 326 lines
    of duplicate prose drifting apart, so the ports share the list and a port
    supplies only its differences.

    IDS ONLY, AND ONLY IDS THAT ALREADY EXIST. The generator owns the
    numbering; an override cannot add, remove or renumber anything. So
    STR_COUNT and strdata.h are identical for every port, and a port's disk can
    never disagree with its binary about what an id means -- which is the
    failure str_load()'s count guard exists to catch and which this makes
    impossible instead. Sources are not scanned and no header is written: this
    mode only re-emits the blob.
    """
    known = load_list()
    if not known:
        raise SystemExit("gen_strings: no %s to override" % LIST)
    n = 0
    for ln in open(over_path):
        ln = ln.rstrip("\n")
        if not ln or ln.startswith("#"):
            continue
        i, _, t = ln.partition("\t")
        i = int(i)
        if i not in known:
            raise SystemExit("gen_strings: %s overrides id %d, which is not in %s"
                             % (over_path, i, LIST))
        if len(t) > MAXLEN:
            raise SystemExit("gen_strings: %s id %d is %d chars, max %d"
                             % (over_path, i, len(t), MAXLEN))
        known[i] = t
        n += 1
    blob = write_blob(known, out_path)
    print("gen_strings: %d strings, %d overridden, %d bytes -> %s"
          % (len(known), n, len(blob), out_path))


def main():
    if "--override" in sys.argv:
        i = sys.argv.index("--override")
        return override_blob(sys.argv[i + 1], sys.argv[i + 2])
    ...
    # The padding after the text is there so a full-width read of the LAST
    # string cannot run off the end of the pool -- S() reads STR_MAX bytes at a
    # time and then looks for the terminator, which is much cheaper than a far
    # read per character. See write_blob().
    blob = write_blob({i: t for t, i in ids.items()}, BLOB)
