...
def main():
    ...
    # The blob is assembled further down, but its SIZE is decided here: every
    # track is its notes plus the (0,0) terminator, two bytes each. Derived
    # rather than measured afterwards so MUS_BYTES cannot disagree with the
    # offsets written from the same sum.
    mus_bytes = sum((len(notes) + 1) * 2 for _, notes, _ in built)
    ...
    with open(hdr, "w") as f:
        ...
        f.write("\n#define MUS_COUNT %d\n\n" % len(built))
        f.write("/* The whole file's size. A port whose far memory is SLOW -- the\n"
                "   card-less CoCo 3 reads its far store off the disk -- lifts all of\n"
                "   MUSIC.DAT into RAM once rather than fetching two bytes a note, and\n"
                "   needs a buffer size that cannot go stale behind the data. */\n")
        f.write("#define MUS_BYTES %d\n\n" % mus_bytes)
    ...
    if len(blob) != mus_bytes:
        sys.exit("make_music: MUS_BYTES says %d, the blob is %d -- the header "
                 "and the data disagree" % (mus_bytes, len(blob)))
