...
def pitches(path):
    ...
    return [r[0] for r in runs if r is not None and r[1] >= 2]
...
def main():
    ...
    want = collapse([t * 10 for _, t in track if t])   # rests make no pitch
    ...
    for region in ("pal", "ntsc"):
        ...
        worst = 0.0
        for i in range(n):
            err = 100.0 * (got[i] - want[i]) / want[i]
            if abs(err) > abs(worst):
                worst = err
            print("   heard %6.0f Hz   want %5d Hz   %+6.2f%%" % (got[i], want[i], err))
