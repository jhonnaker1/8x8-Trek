...
**ALL FIVE ARE CLOSED** -- the plasma bolt, [0x1F31], the fn 0x15F51 jump and
the warp damage on the day they opened; HAIL's slot and the supply trigger the
day after. THE READ LIST IS EMPTY AGAIN, and the last time that was said it
lasted four hours, so: empty AS OF 2026-08-28 EVENING, with no constant and no
mechanism outstanding that anything currently depends on.
  ...
  5. ~~**What raises `[0x24A9 + quadrant]` past 20**~~ **NOTHING does -- and
     THE ANSWER WAS ALREADY IN core/planet.h, read 2026-08-26.** This entry
     was a stale duplicate of a settled question, sitting on the read list
     being counted as work for two days. The genuinely new half, read
     2026-08-28: the only RUNTIME writes are `mod 10` at 0x00E88F and
     0x00E905, which STRIP the find when the landing party collects it --
     `PF_TAKEN`, which this port arrived at independently.
...
with C64 OS's menu bar and status line intact, and the in-image trace byte
reads **7 -- `trek_draw` RETURNED**. The digits are generated by a C `for`
loop doing arithmetic and calling back into the OS once per character, so
they cannot come from anywhere but compiled C. The chain is complete:
llvm-mos compiles C, links a `$0900` app with five vectors and a link table;
C64 OS loads it, relinks the table, calls `init`, accepts the layer push and
calls the draw callback; the C draws through C64 OS's own draw context.

**THE BLANK WINDOW BEFORE IT WAS TWO BUGS, BOTH READABLE IN THE SDK.**
`d_crsr_h` is `$0001`, so **draw-property flags of 0 move the cursor DOWN THE
COLUMN** and skip the PETSCII translation; and `cblack` is 0 while `cwhite`
is 1, so the first build **drew white text on C64 OS's white background**.
The right values are `d_crsr_h|d_petscr` = `$41` and `cblack`, which is
exactly what uno's own draw calls pass. Lowercase on screen is the OS's
charset, not a fault.
