 ...
 * affordance at all. */
/* No kb_inject here. The C128 port needs it because VICE's KEYBOARD_FEED
   fills the KERNAL buffer and that port scans the matrix directly, so fed
   keys never arrive; Xemu has no such problem and a test drives the game
   through $D610 like anything else. */
